...
def compute_episode_stats(episode_data: dict[str, list[str] | np.ndarray], features: dict) -> dict:
    ...
    for key, data in episode_data.items():
        if key not in features:  # 跳过非特征数据
            continue
        ...
        if features[key]["dtype"] == "string":
            ...
            if len(ep_ft_array.shape) == 3:  # [H, W, C]
                ep_ft_array = ep_ft_array[np.newaxis, ...]  # 添加时间维度 [1, H, W, C]
            axes_to_reduce = (0,)  # 只在时间维度上计算统计信息
            keepdims = True
        else:
            # 对于非图像/视频数据，确保是2D数组 [N, D]
            ep_ft_array = np.array(data)
            ...
            axes_to_reduce = (0,)  # 在第一个维度上计算统计信息
            keepdims = True
...
class NavDatasetMetadata(LeRobotDatasetMetadata):
    ...
    def save_episode(
        ...
        """扩展基类的save_episode方法，增加action_config支持"""
...
class NavDataset(LeRobotDataset):
    ...
    def save_episode(self, files: dict) -> None:
        """扩展基类的save_episode方法，支持视频文件复制和图像目录复制"""
        ...
        for key, ft in self.features.items():
            # index, episode_index, task_index已经处理过，image和video单独处理
...
def load_streamvln_episode(
    ...
    """
    加载StreamVLN episode数据，返回LeRobot格式的迭代器
    
    Args:
        ann: 单个annotation字典
        dataset_name: 数据集名称 (EnvDrop/R2R/RxR)
        data_dir: 数据根目录
    ...
    try:
        ...
        ann_id = parts[-1]
        # 修正路径解析逻辑
        # 原格式: "video": "images/17DRP5sb8fy_envdrop_111702"
        # 实际路径: images/17DRP5sb8fy/rgb
        ...
        if not image_files:
            logger.warning(f"在 {src_image_dir} 中没有找到图像文件")
            return
        ...
        logger.error(f"加载episode {ann_id} 失败: {str(e)}", exc_info=True)
        return
...
def process_episode(
    ...
    try:
        ...
        # rel_path = Path(dataset_name.lower()) / episode_id
        ...
        if frame_count > 0:
            ...
            message = f"成功处理: {episode_id}, {frame_count} 帧"
            return (episode_id, True, message)
        else:
            message = f"没有帧被处理，跳过: {episode_id}"
            return (episode_id, False, message)
            
    except Exception as e:
        message = f"处理episode失败: {str(e)}"
...
def process_dataset(
    ...
    if not ann_file.exists():
        logger.error(f"找不到annotation文件: {ann_file}")
        return 0, 0
    ...
    if selected_count == 0:
        logger.warning(f"在索引范围 [{start_idx}, {end_idx}) 内未找到任何episodes")
        return 0, 0
    
    logger.info(
        f"开始处理数据集: {dataset_name} "
        f"(总episodes: {total}, 处理范围: [{start_idx}, {end_idx}), 实际处理: {selected_count})"
    )
    success_count = 0
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        for future in progress_bar:
            ...
            if success:
                success_count += 1
            progress_bar.set_postfix_str(f"成功: {success_count}/{selected_count} "
...
def main(
    ...
    logger.info("=" * 50)
    logger.info("转换完成!")
    logger.info(f"总episodes数: {total_episodes}")
    logger.info(f"成功转换: {success_episodes}")
    logger.info(f"失败: {total_episodes - success_episodes}")
    logger.info("=" * 50)

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="转换StreamVLN数据集到LeRobot格式")
    parser.add_argument("--data_dir", type=str, default="/nav-oss/streamvln", help="StreamVLN数据根目录")
    parser.add_argument("--repo_name", type=str, default="vln_ce_lerobot", help="输出数据集名称")
    parser.add_argument("--output_dir", type=str, help="输出目录（默认使用LEROBOT_HOME）")
    parser.add_argument("--push_to_hub", action="store_true", help="推送到Hugging Face Hub")
    parser.add_argument("--num_threads", type=int, default=10, help="处理线程数")
    parser.add_argument("--start_index", type=int, default=0, help="起始索引（包含）")
    parser.add_argument("--end_index", type=int, default=2000, help="结束索引（不包含）")
